		...
		if (initialise) {
			...
			function init() {
				if (!initialised) {
					...
					let nbSlides = document.querySelectorAll('section[data-menu-title]').length;
					if (nbSlides > 0)
						addToolbarButton('Cours', 'Slides', 'fa-images', 'fas', openPanel, true);
					...
					if (custom) {
						custom.forEach(function(element, index, array) {
							...
							addToolbarButton(element.title, 'Custom' + index, element.icon, null, fn, nbSlides == 0 && index == 0);
						});
					}
					...
					button.appendChild(create('span', {'class': 'slide-menu-toolbar-label'}, 'Fermer'));
					...
					if (custom) {
						...
						custom.forEach(function(element, index, array) {
							var panel = create('div', {
								...
								class: 'slide-menu-panel slide-menu-custom-panel' + ((nbSlides == 0 && index == 0)?' active-menu-panel':'')
							});
							if (element.content) {
					...
					if (openButton) {
						...
						var link = create('a', {href: '#', title : 'Afficher le menu'});
